      ...
      {error ? (
        ...
        <>
          ...
          <section className="overflow-hidden rounded-3xl border border-surface-700/60 bg-surface-900/40">
            <div className="flex items-center justify-between border-b border-surface-700/60 px-5 py-4 bg-surface-900/10">
              <div>
                <h2 className="font-bold text-slate-100 font-sans">Ledger HPP & Margin</h2>
                ...
                <table className="w-full min-w-[680px] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-surface-700/50">
                    {filteredInsights.map((item) => {
                      ...
                      return (
                        <tr key={item.recipe.id} className="text-slate-300 hover:bg-surface-800/10 transition-colors">
                          <td className="px-5 py-4 font-semibold text-slate-100">{item.recipe.name}</td>
                          <td className="px-4 py-4 font-mono text-xs">{formatRupiah(item.totalCost)}</td>
                          <td className="px-4 py-4 font-mono text-xs">
                            {item.sellingPrice ? (
                              formatRupiah(item.sellingPrice)
                            ) : (
                              <span className="font-sans text-slate-500 text-xs italic">Belum diatur</span>
                            )}
                          </td>
                          ...
                          <td className={`px-5 py-4 text-right font-mono font-bold text-xs ${needsAttention ? "text-warm-300" : "text-brand-300"}`}>
...
function AnalysisMetric({
  ...
  return (
    <article className={`rounded-2xl border bg-surface-900/40 p-4 ${accentBorder}`}>
      <Icon className={`h-4.5 w-4.5 ${iconColor}`} />
      <p className="mt-5 text-[10px] font-bold uppercase tracking-wider text-slate-500">{label}</p>
      <p className="mt-1.5 text-lg font-extrabold text-slate-100">{value}</p>
    </article>
  );
}
