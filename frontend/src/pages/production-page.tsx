  ...
  return (
    <div className="space-y-7">
      ...
      <form
        ...
        className="grid gap-4 rounded-3xl border border-surface-700/60 bg-surface-900/40 p-5 sm:grid-cols-[1fr_180px_auto] sm:items-end print:hidden"
      >
        ...
        {filteredBatches.length === 0 ? (
          ...
          {paginatedBatches.map((batch) => {
            ...
            return (
              <article
                key={batch.id}
                className="overflow-hidden rounded-3xl border border-surface-700/60 bg-surface-900/40 print:border-slate-300 print:bg-white print:text-slate-900"
              >
                {/* Batch Header */}
                <div className="flex flex-col gap-3 border-b border-surface-700/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between bg-surface-900/10 print:bg-slate-100 print:border-slate-300">
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-slate-100 print:text-slate-900">{batch.recipe?.name || "Resep produksi"}</p>
                      {batch.notes && (
                        <span className="text-[10px] bg-surface-800 text-slate-400 px-2 py-0.5 rounded-md border border-surface-700/40 print:bg-slate-200 print:text-slate-700 print:border-slate-300">
                          {batch.notes}
                        </span>
                      )}
                    </div>
                ...
                {/* Batch Items list */}
                <div className="grid divide-y divide-surface-700/50 sm:grid-cols-2 lg:grid-cols-3 sm:divide-x sm:divide-y-0 bg-surface-950/20 print:bg-white print:divide-slate-200 print:border-t print:border-slate-300">
