...
export function ProductionPage() {
  ...
  return <div className="space-y-7">
    ...
    <form onSubmit={createBatch} className="grid gap-4 rounded-3xl border border-surface-700/60 bg-surface-900/40 p-5 sm:grid-cols-[1fr_180px_auto] sm:items-end">
      <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">Menu<select value={recipeId} onChange={(event) => handleRecipeChange(event.target.value)} className="mt-2 w-full rounded-xl border border-surface-700 bg-surface-950/40 px-3 py-2.5 text-sm text-slate-200">{menus.map((recipe) => <option key={recipe.id} value={recipe.id}>{recipe.name}</option>)}</select></label>
      <label className="block text-xs font-bold uppercase tracking-wider text-slate-500">Target hasil<input value={targetYield} onChange={(event) => setTargetYield(Number(event.target.value))} type="number" min="0.1" step="0.1" className="mt-2 w-full rounded-xl border border-surface-700 bg-surface-950/40 px-3 py-2.5 text-sm text-slate-200" /></label>
      <button disabled={isSaving || !selectedRecipe} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-bold text-surface-950 disabled:opacity-50"><Plus className="h-4 w-4" /> Buat batch</button>
      <input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Catatan batch (opsional)" className="sm:col-span-2 rounded-xl border border-surface-700 bg-surface-950/40 px-3 py-2.5 text-sm text-slate-200" />
    </form>
